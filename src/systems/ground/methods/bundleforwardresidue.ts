...
function BundleForwardResidue(
  ...
  const TotalIBbfr =
    ArrangeSlashIBbfr + FeedIBbfr + WaitIBbfr + TravelIBbfr + RotateBundlerIBbfr + CutBundleIBbfr;
  ...
  const LoadWeightIIAbfr =
    LoadFractionForBundles * MaxLoadWeightIIAbfr * intermediate.cSlopeSkidForwLoadSize;
  const DistIntermIIAbrf =
    ((LoadWeightIIAbfr / intermediate.residueST) * 43560) / assumption.CTLTrailSpacing;
  const DistOut = Math.min(
    2 * input.deliverToLandingDistance,
    input.deliverToLandingDistance + DistIntermIIAbrf / 2
  );
  const DistIn = Math.max(0, input.deliverToLandingDistance - DistIntermIIAbrf / 2);
  ...
  const CostBundleResidue =
    intermediate.residueST > 0
      ? (machineCost.PMH_Bundler * RelevanceIAbfr +
        ...
        (RelevanceIAbfr * GT1Abfr + RelevanceIBbfr * GT1Bbfr + RelevanceICbfr * GT1Cbfr)
      : 0;
  const HorsepowerBundler = 180;
  const fcrBundler = 0.025;
  const GalBundleResidue =
    (HorsepowerBundler * fcrBundler * CostBundleResidue) / machineCost.PMH_Bundler;
  ...
  const GalForwardResidueBundles =
    (HorsepowerForwarderS * fcrForwarder * CostForwardResidueBundles) / machineCost.PMH_ForwarderS;

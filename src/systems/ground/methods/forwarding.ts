...
function Forwarding(
  ...
  const DistIntermA =
    ((LoadVolA / intermediate.VolPerAcreST) * 43560) /
  ...
  const TravelLoadedA =
    2.5626 + 3.53836 * Math.pow(10, -6) * Math.pow(DistInA, 2);
  const UnloadA = Math.exp(0.61358 + 0.01378 * LoadFraction * 100);
  const TurnTimeA =
    TravelOutA + LoadA + TravelIntermA + TravelLoadedA + UnloadA;
  ...
  const LoadVolB =
    ...
    intermediate.CSlopeSkidForwLoadSize;
  const DistIntermB =
    ((LoadVolB / intermediate.VolPerAcreST) * 43560) /
    assumption.CTLTrailSpacing;
  ...
  const MoveLandingB =
    (0.1763 + 0.0061 * 0.305 * LandingMoveDistPerMoveB) * LandingMovesB;
  const TurnTimeB =
    TravelOutB +
    LoadB +
    SortWoodsB +
  ...
  const LoadVolC =
    ...
    intermediate.CSlopeSkidForwLoadSize;
  const DistIntermC =
    ((LoadVolC / intermediate.VolPerAcreST) * 43560) /
    assumption.CTLTrailSpacing;
  ...
  const LoadVolD =
    ...
    intermediate.CSlopeSkidForwLoadSize;
  const DistIntermD =
    ((LoadVolD / intermediate.VolPerAcreST) * 43560) /
    assumption.CTLTrailSpacing;
  ...
  const TravelOutAndTravelLoadedD =
    (103.36 +
      0.8114 * input.DeliverDist +
      0.0117 * input.DeliverDist * input.Slope) /
    100;
  const LoadD =
    (235.81 +
      Math.max(
        0,
        1549.6 * LoadFraction - 128.48 * LoadFraction * intermediate.CTLLogVol
      )) /
    100;
  const TravelIntermD =
    (51.21 + 0.7519 * DistIntermD + 90.84 * MultipleCorridorDummyD) / 100;
  const TravelLoadedOnRoadD = (36.82 + 0.198 * RoadDistD) / 100;
  const UnloadD =
    (-162 + 852.2 * LoadFraction + 5105 * ColdDummyD * SawlogFractionD) / 100;
  const TurnTimeD =
    TravelEmptyOnRoadD +
  ...
  const RelevanceEforward =
    intermediate.TreeVolST < 10
      ? 1
      : intermediate.TreeVolST < 20
      ? 2 - intermediate.TreeVolST / 10
      : 0;
  ...
  const LoadVolF =
    ...
    intermediate.CSlopeSkidForwLoadSize;
  const DistIntermF =
    ((LoadVolF / intermediate.VolPerAcreST) * 43560) /
    assumption.CTLTrailSpacing;

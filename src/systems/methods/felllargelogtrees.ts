...
function FellLargeLogTrees(
  ...
  // IC (Keatley, 2000)
  const TimePerTreelltC = Math.sqrt(
    4.58 + 0.07 * WalkDistLLT + 0.16 * intermediate.DBHLLT
  );
  ...
  const TimePerTreelltD =
    1.082 + 0.01505 * input.TreeVolLLT - 0.634 / input.TreeVolLLT;
  ...
  const RelevancelltD =
    input.TreeVolLLT < 5
      ...
      : input.TreeVolLLT < 90
      ? 1
      : input.TreeVolLLT < 180
      ? 2 - input.TreeVolLLT / 90
      : 0;
  ...
  const TimePerTreelltIIA =
    EastsideAdjustment *
    (input.PartialCut ? 1 : !input.PartialCut ? ClearcutAdjustment : 0) *
    (1.33 +
      0.0187 * WalkDistLLT +
      0.0143 * input.Slope +
  ...
  const RelevancelltIIB =
    intermediate.TreeVol < 1
      ...
      ? 1
      : 1.2 - intermediate.TreeVol / 350; // ='Felling (all trees)'!E40

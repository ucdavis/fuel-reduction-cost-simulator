...
function CableYarding(input: InputVarMod, intermediate: IntermediateVarMod,
    ...
    function CCSLChangeCost(LineShiftTime: number) {
        ...
        const AreaLineShiftRunning = TailblockSpacingRunning / 2 * 1.5 * input.deliver_dist / 43560;
        const LineShiftCostRunning = 100 * (YarderHourlyCost * LineShiftTime)
            / (intermediate.VolPerAcre * AreaLineShiftRunning);
        const AreaLandingRunning = Math.PI / 2 * Math.pow(1.5 * input.deliver_dist, 2) / 43560;
        const LandingShiftTimeRunning = 2;
        const LandingShiftCostRunning = 100 * (YarderHourlyCost * LandingShiftTimeRunning)
    ...
    const TurnVol = Math.min(YarderCapacity, Math.max(AreaLimitedTurnVol, intermediate.TreeVol));
    const Logs = Math.max(1, TurnVol / intermediate.LogVol);
    function VolPerPMH(TurnTimeCable: number) {
        return TurnVol / (TurnTimeCable / 60);
    }
    function YardingPerCCF(VolPerPMHarg: number) {
        return 100 * YarderHourlyCost / VolPerPMHarg;
    }
    // A) Clearwater Shotgun Live Skyline (Johnson&Lee, 88)
    const TurnEndAreaIIAcable = 2 * TurnVol / assumption.LogLength;
    const DeckHeightIIAcable = 5;
    const TurnTimeIIAcable = 1.51 + 0.012 * TurnVol + 0.011 * DeckHeightIIAcable * Logs
        + 0.021 * DeckHeightIIAcable * TurnEndAreaIIAcable + 0.004 * (input.deliver_dist + LatDist)
        + 0.0002 * Math.pow(LatDist, 2);
    const VolPerPMHIIAcable = VolPerPMH(TurnTimeIIAcable);
    const YardingPerCCFIIAcable = YardingPerCCF(VolPerPMHIIAcable);
    ...
    const TurnTimeIIBcable = 1.82 + 0.0024 * input.deliver_dist + 0.00021 * Math.pow(LatDist, 2)
        + 0.0125 * TurnVol + 0.0151 * input.Slope + 1.05;
    const VolPerPMHIIBcable = VolPerPMH(TurnTimeIIBcable);
    const YardingPerCCFIIBcable = YardingPerCCF(VolPerPMHIIBcable);
    const CostPerCCFIIBcable = YardingPerCCFIIBcable + PCLiveStandSLChangeCost;
    const RelevanceIIBcable = TurnVol < 200 ? 1 : (TurnVol < 400 ? 2 - TurnVol / 200 : 0);
    ...
    const TurnTimeIICcable = Math.exp(1.46 + 0.000486 * input.deliver_dist + 0.00114 * LatDist
        + 0.00000896 * intermediate.WoodDensity * TurnVol);
    const VolPerPMHIICcable = VolPerPMH(TurnTimeIICcable);
    const YardingPerCCFIICcable = YardingPerCCF(VolPerPMHIICcable);
    ...
    const TurnTimeIIDcable = Math.exp(0.58 + 0.192 * Math.log(input.deliver_dist)
        - 0.00308 * input.Slope + 0.00193 * LatDist + 0.000004 * Logs * intermediate.WoodDensity * TurnVol);
    const VolPerPMHIIDcable = VolPerPMH(TurnTimeIIDcable);
    const YardingPerCCFIIDcable = YardingPerCCF(VolPerPMHIIDcable);
    ...
    const TurnTimeIIEcable = Math.exp(1.813 + 0.00094 * input.deliver_dist - 0.0095 * input.Slope
        + 0.00172 * LatDist + 0.00000277 * Logs * intermediate.WoodDensity * TurnVol);
    const VolPerPMHIIEcable = VolPerPMH(TurnTimeIIEcable);
    const YardingPerCCFIIEcable = YardingPerCCF(VolPerPMHIIEcable);
    ...
    const VolPerPMHIIFcable = VolPerPMH(TurnTimeIIFcable);
    const YardingPerCCFIIFcable = YardingPerCCF(VolPerPMHIIFcable);
    ...
    const VolPerPMHIIGcable = VolPerPMH(TurnTimeIIGcable);
    const YardingPerCCFIIGcable = YardingPerCCF(VolPerPMHIIGcable);
    ...
    const TurnTimeIIHcable = 2.341 + 0.0017 * input.deliver_dist + 0.012 * LatDist + 0.722 * PresetIIHcable
        + 0.191 * Logs + 0.334 * SpanIIHcable + 0.17 * DamageIIHcable;
    const VolPerPMHIIHcable = VolPerPMH(TurnTimeIIHcable);
    const YardingPerCCFIIHcable = YardingPerCCF(VolPerPMHIIHcable);
    ...
    const TurnTimeIIIcable = 0.231 + 0.003 * input.deliver_dist + 0.01 * LatDist + 0.017 * CarriageHtIIIcable
        + 0.141 * Logs + 0.012 * input.Slope - 0.668 * PresetIIIcable;
    const VolPerPMHIIIcable = VolPerPMH(TurnTimeIIIcable);
    const YardingPerCCFIIIcable = YardingPerCCF(VolPerPMHIIIcable);
    ...
    const TurnTimeIIJcable = 0.223 + 0.004 * input.deliver_dist + 0.024 * LatDist
        + 0.059 * TurnVol + 35.23 / intermediate.TreeVol;
    const VolPerPMHIIJcable = VolPerPMH(TurnTimeIIJcable);
    const YardingPerCCFIIJcable = YardingPerCCF(VolPerPMHIIJcable);
    ...
    const YardingPerCCFIIKcable = YardingPerCCF(VolPerPMHIIKcable);
    ...
    const YardingPerCCFIILcable = YardingPerCCF(VolPerPMHIILcable);
    ...
    const YardingPerCCFIIMcable = YardingPerCCF(VolPerPMHIIMcable);
    ...
    const YardingPerCCFIINcable = YardingPerCCF(VolPerPMHIINcable);
    ...
    const YardingPerCCFIIOcable = YardingPerCCF(VolPerPMHIIOcable);
    ...
    const VolPerPMHIIPcable = VolPerPMH(TurnTimeIIPcable);
    const YardingPerCCFIIPcable = YardingPerCCF(VolPerPMHIIPcable);
    ...
    return { 'CostYardPCUB': CostYardPCUB };
}

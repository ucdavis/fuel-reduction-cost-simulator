...
function FellBunch(input: InputVarMod, intermediate: IntermediateVarMod, machineCost: MachineCostMod) {
/*--------------Fell&Bunch START---------------------------*/
const PMH_DriveToTree = machineCost.PMH_DriveToTree;
const DistBetweenTrees = Math.sqrt(43560 / Math.max(intermediate.RemovalsST, 1));
// I. Drive-To-Tree
// IA: Melroe Bobcat (Johnson, 79)
const TimePerTreeIA = 0.204 + 0.00822 * DistBetweenTrees + 0.02002 * intermediate.DBHST + 0.00244 * input.Slope;
...
const RelevanceIA = (intermediate.DBHST < 10 ? 1 : (intermediate.DBHST < 15 ? 3 - intermediate.DBHST / 5 : 0))
* (input.Slope < 10 ? 1 : (input.Slope < 20 ? 2 - input.Slope / 10 : 0));
// IB: Chainsaw Heads START
const CutsIB = 1.1;
const TimePerTreeIB = (-0.0368 + 0.02914 * intermediate.DBHST + 0.00289 * DistBetweenTrees + 0.2134 * CutsIB)
...
const RelevanceIB = (intermediate.DBHST < 15 ? 1 : (intermediate.DBHST < 20 ? 4 - intermediate.DBHST / 5 : 0))
* (input.Slope < 10 ? 1 : (input.Slope < 20 ? 2 - input.Slope / 10 : 0));
// IC: Intermittent Circular Sawheads (Greene&McNeel, 91)
const CutsIC = 1.01;
const TimePerTreeIC = (-0.4197 + 0.01345 * intermediate.DBHST + 0.001245 * DistBetweenTrees + 0.7271 * CutsIC)
* (1 + intermediate.CSlopeFB_Harv);
...
const RelevanceIC = (intermediate.DBHST < 15 ? 1 : (intermediate.DBHST < 20 ? 4 - intermediate.DBHST / 5 : 0))
* (input.Slope < 10 ? 1 : (input.Slope < 20 ? 2 - input.Slope / 10 : 0));
// ID: Hydro-Ax 211 (Hartsough, 01)
const TreesPerAccumID = Math.max(1, 14.2 - 2.18 * intermediate.DBHST + 0.0799 * Math.pow(intermediate.DBHST, 2));
const TimePerAccumID = 0.114 + 0.266 + 0.073 * TreesPerAccumID + 0.00999 * TreesPerAccumID * intermediate.DBHST;
...
const RelevanceID = (intermediate.DBHST < 10 ? 1 : (intermediate.DBHST < 15 ? 3 - intermediate.DBHST / 5 : 0))
* (input.Slope < 10 ? 1 : (input.Slope < 20 ? 2 - input.Slope / 10 : 0));
// II. Swing Boom
...
const TreeInReachIIB = intermediate.RemovalsST * Math.PI * Math.pow(BoomReachIIB, 2) / 43560;
const TreesPerCycleIIB = Math.max(1, TreeInReachIIB);
const TimePerCycleIIB = (0.242 + 0.1295 * TreesPerCycleIIB + 0.0295 * intermediate.DBHST * TreesPerCycleIIB)
* (1 + intermediate.CSlopeFB_Harv);
const TimePerTreeIIB = TimePerCycleIIB / TreesPerCycleIIB;
const VolPerPMHIIB = intermediate.TreeVolST / (TimePerTreeIIB / 60);
const CostPerPMHIIB = PMH_SwingBoom * intermediate.NonSelfLevelCabDummy
    + PMH_SelfLevel * (1 - intermediate.NonSelfLevelCabDummy);
const CostPerCCFIIB = 100 * CostPerPMHIIB / VolPerPMHIIB;
const RelevanceIIB = (intermediate.DBHST < 15 ? 1 : (intermediate.DBHST < 20 ? 4 - intermediate.DBHST / 5 : 0))
* (input.Slope < 5 ? 0 : (input.Slope < 20 ? -1 / 3 + input.Slope / 15 : 1));
// IIC: JD 693B&TJ Timbco 2518 (Gingras, 88)
const UnmerchTreesPerHaIIC = 285;
const UnmerchPerMerchIIC = Math.min(1.5, 285 / (2.47 * intermediate.RemovalsST));
const BoomReachIIC = 24;
const TreesInReachIIC = intermediate.RemovalsST * Math.PI * Math.pow(BoomReachIIC, 2) / 43560;
const ObsTreesPerCycleIIC = (4.36 + 9 - (0.12 + 0.34) * intermediate.DBHST
    + 0.00084 * 2.47 * intermediate.RemovalsST) / 2;
...
const CostPerPMHIIC = PMH_SwingBoom * intermediate.NonSelfLevelCabDummy
    + PMH_SelfLevel * (1 - intermediate.NonSelfLevelCabDummy);
const CostPerCCFIIC = 100 * CostPerPMHIIC / VolPerPMHIIC;
const RelevanceIIC = (intermediate.DBHST < 12 ? 1 : (intermediate.DBHST < 18 ? 3 - intermediate.DBHST / 6 : 0))
* (input.Slope < 5 ? 0 : (input.Slope < 20 ? -1 / 3 + input.Slope / 15 : 1));
// IID: Timbco (Gonsier&Mandzak, 87)
const TimePerTreeIID = (0.324 + 0.00138 * Math.pow(intermediate.DBHST, 2))
...
const RelevanceIID = (intermediate.DBHST < 15 ? 1 : (intermediate.DBHST < 20 ? 4 - intermediate.DBHST / 5 : 0))
...
const VolPerPMHIIE = (50.338 / 0.028317 * Math.pow((intermediate.TreeVolST * 0.028317), 0.3011))
/ (1 + intermediate.CSlopeFB_Harv + intermediate.CRemovalsFB_Harv);
const CostPerPMHIIE = PMH_SwingBoom * intermediate.NonSelfLevelCabDummy
    + PMH_SelfLevel * (1 - intermediate.NonSelfLevelCabDummy);
...
const VolPerPMHIIF = (5 / 0.028317 + 57.7 * intermediate.TreeVolST)
    / (1 + intermediate.CSlopeFB_Harv + intermediate.CRemovalsFB_Harv);
const CostPerPMHIIF = PMH_SwingBoom * intermediate.NonSelfLevelCabDummy
    + PMH_SelfLevel * (1 - intermediate.NonSelfLevelCabDummy);
const CostPerCCFIIF = 100 * CostPerPMHIIF / VolPerPMHIIF;
const RelevanceIIF = (intermediate.TreeVolST < 20 ?
    1 : (intermediate.TreeVolST < 50 ? 5 / 3 - intermediate.TreeVolST / 30 : 0))
* (input.Slope < 5 ? 0 : (input.Slope < 20 ? -1 / 3 + input.Slope / 15 : 1));
...
const TreesInReachIIG = intermediate.RemovalsST * Math.PI * Math.pow(BoomReachIIG, 2) / 43560;
const TreesPerAccumIIG = Math.max(1, 1.81 - 0.0664 * intermediate.DBHST
    + 3.64 / intermediate.DBHST - 0.0058 * 20 - 0.27 * 0 - 0.1 * 0);
...
const FellIIG = 0.285 + 0.126 * TreesPerAccumIIG + 0.0176 * intermediate.DBHST * TreesPerAccumIIG - 0.0394 * DeadIIG;
const TimePerAccumIIG = MoveFracIIG * MoveIIG + FellIIG;
const TimePerTreeIIG = (TimePerAccumIIG * (1 + DelayFracIIG) / TreesPerAccumIIG) * (1 + intermediate.CSlopeFB_Harv);
const VolPerPMHIIG = intermediate.TreeVolST / TimePerTreeIIG * 60;
const CostPerPMHIIG = PMH_SwingBoom * intermediate.NonSelfLevelCabDummy
    + PMH_SelfLevel * (1 - intermediate.NonSelfLevelCabDummy);
const CostPerCCFIIG = 100 * CostPerPMHIIG / VolPerPMHIIG;
const RelevanceIIG = (intermediate.DBHST < 15 ? 1 : (intermediate.DBHST < 20 ? 4 - intermediate.DBHST / 5 : 0))
* (input.Slope < 5 ? 0 : (input.Slope < 20 ? -1 / 3 + input.Slope / 15 : 1));
...
// Summary
const CostFellBunch = (intermediate.TreeVolST > 0 ? intermediate.CHardwoodST * 100

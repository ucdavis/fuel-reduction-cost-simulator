...
function FellAllTrees(input: InputVarMod, intermediate: IntermediateVarMod, machineCost: MachineCostMod) {
    ...
    // B) (Peterson, 87)
    const TimePerTreeIBat = intermediate.DBH < 10 ? 0.33 + 0.012 * intermediate.DBH : 0.1
        + 0.0111 * Math.pow(intermediate.DBH, 1.496);
    ...
    const TimePerTreeIDat = 1.082 + 0.01505 * intermediate.TreeVol - 0.634 / intermediate.TreeVol;
    ...
    const RelevanceIDat = intermediate.TreeVol < 0.6 ?
        0 : (intermediate.TreeVol < 15 ? 1 - (15 / (15 - 0.6)) + (intermediate.TreeVol / (15 - 0.6))
        : (intermediate.TreeVol < 90 ? 1 : (intermediate.TreeVol < 180 ? 2 - intermediate.TreeVol / 90 : 0)));
    ...
    // Summary
    const CostManFell = intermediate.CHardwood * 100 * (machineCost.PMH_Chainsaw * RelevanceIAat
        + machineCost.PMH_Chainsaw * RelevanceIBat + machineCost.PMH_Chainsaw * RelevanceICat
        + machineCost.PMH_Chainsaw * RelevanceIDat + machineCost.PMH_Chainsaw * RelevanceIEat)
    ...
    const TimePerTreeIIBat = (-0.465 + 0.102 * intermediate.DBH + 0.016 * LimbsIIBat
        + 0.562 * LogsIIBat + 0.009 * input.Slope + 0.734 * WedgeIIBat + 0.137 * CorridorIIBat
        + 0.449 * NotBetweenOpeningsIIBat + 0.437 * OpeningsIIBat
    ...
    const RelevanceIIBat = intermediate.TreeVol < 1 ? 0
        : (intermediate.TreeVol < 2 ? -1 + intermediate.TreeVol / 1
            : (intermediate.TreeVol < 70 ? 1 : 1.2 - intermediate.TreeVol / 350));
    ...
    const TimePerTreeIICat = (1.772 + 0.02877 * intermediate.TreeVol
        - 2.6486 / intermediate.TreeVol) * (1 + DelayFracIICat);
    ...
    const RelevanceIICat = intermediate.TreeVol < 5 ?
        0 : (intermediate.TreeVol < 15 ? -0.5 + intermediate.TreeVol / 10 : 1);
    // Weight relaxed at upper end to allow extrapolation to larger trees. Original was
    ...
    // Summary
    const CostManFLB = intermediate.CHardwood * 100 * (machineCost.PMH_Chainsaw * RelevanceIIAat
        + machineCost.PMH_Chainsaw * RelevanceIIBat + machineCost.PMH_Chainsaw * RelevanceIICat
        + machineCost.PMH_Chainsaw * RelevanceIIDat) / (RelevanceIIAat * VolPerPMHIIAat

...
function InLimits(input: InputVarMod, intermediate: IntermediateVarMod) {
    ...
    const ExceededMaxLLT = (limit.MaxLLTperAcre === 0 && limit.MaxLLTasPercentALT === 0) ? 0
        : ((input.RemovalsLLT > limit.MaxLLTperAcre
            || 100 * input.RemovalsLLT / (intermediate.RemovalsALT > 0 ?
                intermediate.RemovalsALT : 1) > limit.MaxLLTasPercentALT)
            ? 1 : 0);
    const ExceededMaxTreeVol
    = (input.TreeVolCT > limit.AvgTreeSizeLimit4Chipping || input.TreeVolSLT > limit.AvgTreeSizeLimit4Processing
    || input.TreeVolLLT > limit.AvgTreeSizeLimit4ManualFellLimbBuck
    || intermediate.TreeVolALT > limit.AvgTreeSizeLimit4loading
    || intermediate.TreeVol > limit.AvgTreeSize4GrappleSkidding) ? 1 : 0;
    // Slope, %

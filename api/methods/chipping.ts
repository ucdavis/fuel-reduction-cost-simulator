...
function Chipping(assumption: AssumptionMod, input: InputVarMod,
    ...
    const TreeWeightDry = input.TreeVolCT * intermediate.WoodDensityCT * (1 - assumption.MoistureContent);
    ...
    const ChipperHourlyCost = PMH_ChipperS * (1 - intermediate.ChipperSize) + PMH_ChipperB * intermediate.ChipperSize;

    // I. Chip Whole Trees
    // A) (Johnson, 89)
    const ChipperHP1A = Math.min(700, Math.max(200, 100 + 100 * Math.sqrt(input.TreeVolCT)));
    ...
    const VolPerPMHchippingIB = Math.min(4000, 463 * Math.pow(input.TreeVolCT, 0.668));
    ...
    const VolPerPMHchippingIC = assumption.LoadWeightChip / (intermediate.WoodDensityCT / 2000) / (TimePerVanIC / 60);
    ...
    const VolPerPMHchippingIIIA = assumption.LoadWeightChip
        / (intermediate.WoodDensityCT / 2000) / (TimePerVanIIIA / 60);
    ...
    const VolPerPMHchippingIIIB = assumption.LoadWeightChip
        / (intermediate.WoodDensityCT / 2000) / (TimePerVanIIIB / 60);
    ...
    // Chipping Summary
    // I. Chip Whole Trees
    const CostChipWT = input.TreeVolCT > 0 ? intermediate.CHardwoodCT * 100 * (ChipperHourlyCost * RelevanceChippingIA
    ...
    // result
    const CostDDChipWT = input.TreeVolCT > 0 ? intermediate.CHardwoodCT * 100
    * (CostPerPMHchippingIIA * RelevanceChippingIIA + ChipperHourlyCost * RelevanceChippingIIB)
    ...
    // III. Chip CTL Logs
    const CostChipCTL = input.TreeVolCT > 0 ? intermediate.CHardwoodCT * 100

...
function Chipping(TreeVolCT: number, WoodDensityCT: number, LoadWeightChip: number, MoistureContent: number,
                  CHardwoodCT: number, CostMachine: CostMachineMod, CTLLogVolCT: number, ChipperSize: number) {
const ExchangeVans = 5.3;

// Chipping Calculated Values
const PMH_LoaderS = CostMachine.PMH_LoaderS;
const PMH_ChipperS = CostMachine.PMH_ChipperS;
const PMH_ChipperB = CostMachine.PMH_ChipperB;
const LoadWeightDry = LoadWeightChip * (1 - MoistureContent);
const TreeWeightDry = TreeVolCT * WoodDensityCT * (1 - MoistureContent);
const CTLLogWeight = CTLLogVolCT * WoodDensityCT;
const CTLLogWeightDry = CTLLogWeight * (1 - MoistureContent);
const ChipperHourlyCost = PMH_ChipperS * (1 - ChipperSize) + PMH_ChipperB * ChipperSize;

// I. Chip Whole Trees
// A) (Johnson, 89)
const ChipperHP1A = Math.min(700, Math.max(200, 100 + 100 * Math.sqrt(TreeVolCT)));
const GTperPMHchippingIA = -17 + ChipperHP1A / 6;
const VolPerPMHchippingIA = GTperPMHchippingIA * 2000 / WoodDensityCT;
...
const VolPerPMHchippingIB = Math.min(4000, 463 * Math.pow(TreeVolCT, 0.668));
...
const SlashIC = 0.93;
const TimePerVanIC = ChipTimePerSwingIC * (1 + ProbDelayFractionIC)
/ (TreeWeightDry * LogsPerSwingIC) * 2000 * LoadWeightDry + (SlashIC + ExchangeVans);
const VolPerPMHchippingIC = LoadWeightChip / (WoodDensityCT / 2000) / (TimePerVanIC / 60);
const CostPerCCFchippingIC = 100 * ChipperHourlyCost / VolPerPMHchippingIC;
const RelevanceChippingIC = TreeWeightDry < 400 ? 1 : (TreeWeightDry < 800 ? 2 - TreeWeightDry / 400 : 0);
...
const TimePerGTchippingIIIA = Math.max(0.8, (2.05 - 0.00541 * CTLLogWeight) * (1 + ProbDelayFractionIIIA));
const TimePerVanIIIA = TimePerGTchippingIIIA * LoadWeightChip + ExchangeVans;
const VolPerPMHchippingIIIA = LoadWeightChip / (WoodDensityCT / 2000) / (TimePerVanIIIA / 60);
const CostPerCCFchippingIIIA = 100 * ChipperHourlyCost / VolPerPMHchippingIIIA;
const RelevanceChippingIIIA = Math.max(0.1, CTLLogWeight < 100 ? 1
: (CTLLogWeight < 200 ? 2 - CTLLogWeight / 100 : 0));
...
const TimePerVanIIIB = ChipTimePerSwingIIIB * (1 + ProdDelayFractionIIIB)
/ (CTLLogWeightDry * LogsPerSwingIIIB) * 2000 * LoadWeightDry + (SlashIIIB + ExchangeVans);
const VolPerPMHchippingIIIB = LoadWeightChip / (WoodDensityCT / 2000) / (TimePerVanIIIB / 60);
const CostPerCCFchippingIIIB = 100 * ChipperHourlyCost / VolPerPMHchippingIIIB;
const RelevanceChippingIIIB = CTLLogWeightDry < 400 ? 1 : (CTLLogWeightDry < 800 ? 2 - CTLLogWeightDry / 400 : 0);
...
const GTperPMHchippingIVA = BDTperPMHchippingIVAavg / MoistureContent;
...
// Chipping Summary
// I. Chip Whole Trees
const CostChipWT = TreeVolCT > 0 ? CHardwoodCT * 100 * (ChipperHourlyCost * RelevanceChippingIA
+ ChipperHourlyCost * RelevanceChippingIB + ChipperHourlyCost * RelevanceChippingIC
+ ChipperHourlyCost * RelevanceChippingID)
/ (RelevanceChippingIA * VolPerPMHchippingIA + RelevanceChippingIB * VolPerPMHchippingIB
+ RelevanceChippingIC * VolPerPMHchippingIC + RelevanceChippingID * VolPerPMHchippingID) : 0;
...
const RelevanceChippingIIA = Math.max(RelevanceChippingIA, RelevanceChippingIB, RelevanceChippingIC);
// result
const CostDDChipWT = TreeVolCT > 0 ? CHardwoodCT * 100
* (CostPerPMHchippingIIA * RelevanceChippingIIA + ChipperHourlyCost * RelevanceChippingIIB)
/ (RelevanceChippingIIA * VolPerPMHchippingIIA + RelevanceChippingIIB * VolPerPMHchippingIIB) : 0;
// III. Chip CTL Logs
const CostChipCTL = TreeVolCT > 0 ? CHardwoodCT * 100
* (ChipperHourlyCost * RelevanceChippingIIIA + ChipperHourlyCost * RelevanceChippingIIIB
+ ChipperHourlyCost * RelevanceChippingIIIC)
/ (RelevanceChippingIIIA * VolPerPMHchippingIIIA + RelevanceChippingIIIB * VolPerPMHchippingIIIB
  + RelevanceChippingIIIC * VolPerPMHchippingIIIC) : 0;
// IV. Chip Piled Loose Residues at Landing
const CostChipLooseRes = (CostPerPMHchippingIVA * RelevanceChippingIVA
...
// V. Chip Bundles of Residue at Landing
const CostChipBundledRes = (CostPerPMHchippingIVA * RelevanceChippingVA
+ CostPerPMHchippingIVA * RelevanceChippingVB)
/ (RelevanceChippingVA * GTperPMHchippingVA + RelevanceChippingVB * GTperPMHchippingVB);

const results = { 'CostChipWT': CostChipWT, 'CostDDChipWT': CostDDChipWT, 'CostChipCTL': CostChipCTL,
                  'CostChipLooseRes': CostChipLooseRes, 'CostChipBundledRes': CostChipBundledRes };
// console.log(results);
return results;
}

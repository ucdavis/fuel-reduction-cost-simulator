...
function CYCCU(input: InputVarMod, intermediate: IntermediateVarMod,
    ...
    const TurnTimeIAcable = 0.321 + 0.00285 * input.deliver_dist + 0.009906 * input.Slope;
    ...
    const TurnTimeIBcable = (72.26 + 0.0638 * intermediate.BFperCF * intermediate.LogVol
        + 0.1152 * input.deliver_dist - 996 / Math.max(20, input.Slope)) / 60;
    ...
    const TurnTimeICcable = (177.3 + 0.3568 * intermediate.LogVol + 0.000522 * Math.pow(input.deliver_dist, 2)
    ...
    const TurnTimeIDcable = (164 + 0.04872 * intermediate.BFperCF * intermediate.LogVol
        + 0.000405 * Math.pow(input.deliver_dist, 2) + 607 / Math.max(20, input.Slope)) / 60;
    ...
    const LatDistCC = 5;
    const TurnTimeIEcable = Math.exp(1.09 + 0.0196 * LogsII + 0.00106 * input.deliver_dist
    ...
    const TurnTimeIFcable = Math.exp(1.91 + 0.000545 * input.deliver_dist - 0.0068 * input.Slope
    ...
    const TurnTimeIGcable = (1 + DelayFracIGcable) * (0.31 + 0.00323 * input.deliver_dist

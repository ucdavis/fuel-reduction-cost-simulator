...
function CYCCU(input: InputVarMod, intermediate: IntermediateVarMod,
    ...
    const AreaLineShiftJammer = TailblockSpacingJammer * 2 * input.DeliverDist / 43560;
    ...
    function CCSLChangeCost(LineShiftTime: number) {
        ...
        const AreaLineShiftRunning = TailblockSpacingRunning / 2 * 1.5 * input.DeliverDist / 43560;
        const LineShiftCostRunning = 100 * (YarderHourlyCost * LineShiftTime)
            / (intermediate.VolPerAcre * AreaLineShiftRunning);
        const AreaLandingRunning = Math.PI / 2 * Math.pow(1.5 * input.DeliverDist, 2) / 43560;
    ...
    const TurnTimeIAcable = 0.321 + 0.00285 * input.DeliverDist + 0.009906 * input.Slope;
    ...
    const RelevanceIAcable = intermediate.LogVol < 5 ?
        0 : (intermediate.LogVol < 10 ? -1 + intermediate.LogVol / 5 : (intermediate.LogVol < 50 ?
            1 : (intermediate.LogVol < 100 ? 2 - intermediate.LogVol / 50 : 1)));
    ...
    const TurnTimeIBcable = (72.26 + 0.0638 * intermediate.BFperCF * intermediate.LogVol
        + 0.1152 * input.DeliverDist - 996 / Math.max(20, input.Slope)) / 60;
    ...
    const RelevanceIBcable = intermediate.LogVol < 5 ?
        0 : (intermediate.LogVol < 10 ? -1 + intermediate.LogVol / 5 : 1);
    ...
    const TurnTimeICcable = (177.3 + 0.3568 * intermediate.LogVol + 0.000522 * Math.pow(input.DeliverDist, 2)
        + 0.0105 * Math.pow(input.Slope, 2)) / 60;
    ...
    const RelevanceICcable = intermediate.LogVol < 5 ?
        0 : (intermediate.LogVol < 10 ? -1 + intermediate.LogVol / 5 : 1);
    ...
    const TurnTimeIDcable = (164 + 0.04872 * intermediate.BFperCF * intermediate.LogVol
        + 0.000405 * Math.pow(input.DeliverDist, 2) + 607 / Math.max(20, input.Slope)) / 60;
    ...
    const RelevanceIDcable = intermediate.LogVol < 5 ?
        0 : (intermediate.LogVol < 10 ? -1 + intermediate.LogVol / 5 : 1);
    ...
    const LatDistCC = 5;
    const TurnTimeIEcable = Math.exp(1.09 + 0.0196 * LogsII + 0.00106 * input.DeliverDist
        + 0.000617 * TurnVol2 + 0.000043 * LatDistCC * input.Slope);
    ...
    const RelevanceIEcable = intermediate.LogVol < 30 ?
    ...
    const TurnTimeIFcable = Math.exp(1.91 + 0.000545 * input.DeliverDist - 0.0068 * input.Slope
        + 0.00212 * TurnVol2 - 0.416 / Math.max(1, LogsII));
    ...
    const RelevanceIFcable = (intermediate.LogVol < 30 ?
        1 : (intermediate.LogVol < 60 ? 2 - intermediate.LogVol / 30 : 0)) * (input.Slope < 30 ?
            0 : (input.Slope < 40 ? input.Slope / 10 - 3 : 1));
    ...
    const TurnTimeIGcable = (1 + DelayFracIGcable) * (0.31 + 0.00323 * input.DeliverDist
        + 1.593 + 1.24 * Math.log10(LogsII) + 0.326 + 0.107 * LogsII + 0.021);
    ...
    const RelevanceIGcable = intermediate.TreeVol < 80 ?
        1 : (intermediate.TreeVol < 160 ? 2 - intermediate.TreeVol / 80 : 0);
    ...
    // Results
    const WeightingProduct1Sum = WeightingProductIAcable + WeightingProductIBcable + WeightingProductICcable
        + WeightingProductIDcable + WeightingProductIEcable + WeightingProductIFcable + WeightingProductIGcable
        + WeightingProductIHcable;
    const WeightingDivisor1Sum = WeightingDivisorIAcable + WeightingDivisorIBcable + WeightingDivisorICcable
        + WeightingDivisorIDcable + WeightingDivisorIEcable + WeightingDivisorIFcable + WeightingDivisorIGcable

...
function Loading(assumption: AssumptionMod, input: InputVarMod,
    ...
    const LoaderHourlyCost = PMH_LoaderS * (1 - intermediate.ManualMachineSizeALT)
    ...
    const RelevanceLoadingIA = intermediate.LogVolALT < 10 ?
        0 : (intermediate.LogVolALT < 40 ? -1 / 3 + intermediate.LogVolALT / 30 : 1);
    ...
    const RelevanceLoadingIB = intermediate.LogVolALT < 10 ?
        1 : (intermediate.LogVolALT < 20 ? 2 - intermediate.LogVolALT / 10 : 0);
    ...
    const VolPerPMHloadingID = 100 * (11.04 + 0.522 * intermediate.LogVolALT - 0.00173
        * Math.pow(intermediate.LogVolALT, 2));
    const CostPerCCFloadingID = 100 * LoaderHourlyCost / VolPerPMHloadingID;
    const RelevanceLoadingID = intermediate.LogVolALT < 75 ?
        1 : (intermediate.LogVolALT < 100 ? 4 - intermediate.LogVolALT / 25 : 0);
    ...
    const RelevanceLoadingIIA = intermediate.CTLLogVol < 10 ?
        1 : (intermediate.CTLLogVol < 20 ? 2 - intermediate.CTLLogVol / 10 : 0);
    // B) Loaders (Jackson et al, 84)
    const VolPerPMHloadingIIB = 100 * (11.04 + 0.522 * intermediate.CTLLogVol - 0.00173
        * Math.pow(intermediate.CTLLogVol, 2));

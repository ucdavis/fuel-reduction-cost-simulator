...
function Harvesting(assumption: AssumptionMod, input: InputVarMod,
                    intermediate: IntermediateVarMod, machineCost: MachineCostMod) {

    // Harvesting Calculated Values
    const HarvesterHourlyCost = machineCost.PMH_HarvS * (1 - intermediate.MechMachineSize)
    ...
    const TimePerTreeIAharvest = Math.pow(10, (-1.0123 + 0.219 * intermediate.CTLLogsPerTree
    ...
    const RelevanceIAharvest = intermediate.TreeVolST < 25 ?
    ...
    const TreesInReachIBharvest = intermediate.RemovalsST * assumption.CTLTrailSpacing * BoomReachIBharvest / 2 / 43560;
    ...
    const RelevanceIBharvest = intermediate.TreeVolST < 25 ?
        1 : (intermediate.TreeVolST < 50 ? 2 - intermediate.TreeVolST / 25 : 0);
    // C) Timbco T425/Pika 600 (Schroder&Johnson, 97)
    const BoomReachICharvest = 24;
    const TreesInReachICharvest = intermediate.RemovalsST * assumption.CTLTrailSpacing * BoomReachICharvest / 2 / 43560;
    ...
    const RelevanceICharvest = intermediate.TreeVolST < 25 ?
        1 : (intermediate.TreeVolST < 50 ? 2 - intermediate.TreeVolST / 25 : 0);
    // D) Timberjack 2518 (Kellogg&Bettinger, 94)
    const VolPerPMHIDharvest = (-617 + 189.3 * intermediate.DBHST)
        / (1 + intermediate.CSlopeFB_Harv + intermediate.CRemovalsFB_Harv);
    const CostPerCCFIDharvest = 100 * HarvesterHourlyCost / VolPerPMHIDharvest;
    const RelevanceIDharvest = intermediate.TreeVolST < 40 ?
    ...
    const DistPerMoveIGharvest = Math.max(BoomReachIGharvest / 2,
                                          43560 / assumption.CTLTrailSpacing / intermediate.RemovalsST);
    const TreesInReachIGharvest = intermediate.RemovalsST * assumption.CTLTrailSpacing * BoomReachIGharvest / 2 / 43560;
    const TreesPerMoveIGharvest = Math.max(1, TreesInReachIGharvest);
    const BrushIGharvest = 0.89 / 100;
    const MoveIGharvest = (3.44 + (1.22 + 1.02 * YarderDummyIGharvest + 0.0369 * input.Slope) * DistPerMoveIGharvest)
        / 100 / TreesPerMoveIGharvest;
    const SwingIGharvest = (14.59 + 0.272 * intermediate.DBH - 2.38 * DownDummy) / 100;
    const FellIGharvest = (2.67 + (0.472 - 0.1 * DeadDummy + 0.664 * YarderDummyIGharvest) * intermediate.DBHST)
        / 100 * (1 - DownDummy);
    const ProcessIGharvest = (-4.3 + 2.104 * intermediate.DBHST + (6.5 - 1.17 * DeadDummy + 2.11 * DownDummy
        + 1.66 * YarderDummyIGharvest) * intermediate.CTLLogsPerTree) / 100;
    const TimePerTreeIGharvest = (BrushIGharvest + MoveIGharvest + SwingIGharvest + FellIGharvest + ProcessIGharvest)
        * (1 + ProdDelayFractionIGharvest) * (1 + intermediate.CSlopeFB_Harv);
    const VolPerPMHIGharvest = VolPerPMHfunc(TimePerTreeIGharvest);
    const CostPerCCFIGharvest = CostPerCCFfunc(VolPerPMHIGharvest);
    const RelevanceIGharvest = intermediate.DBHST < 12 ?
        1 : (intermediate.DBHST < 20 ? 2.5 - intermediate.DBHST / 8 : 0);
    // E) Rottne (McNeel&Rutherford, 94)
    const BoomReachIEharvest = 24;
    const DistPerMoveIEharvest = Math.max(BoomReachIEharvest / 2,
                                          43560 / assumption.CTLTrailSpacing / intermediate.RemovalsST);
    const TreesInReachIEharvest = intermediate.RemovalsST * assumption.CTLTrailSpacing * BoomReachIEharvest / 2 / 43560;
    ...
    const TimePerTreeIEharvest = (CutIEharvest + ProcessIEharvest + PositionHeadIEharvest + SlashDisposalIEharvest
        + MoveIEharvest) * (1 + intermediate.CSlopeFB_Harv);
    const VolPerPMHIEharvest = VolPerPMHfunc(TimePerTreeIEharvest);
    const CostPerCCFIEharvest = CostPerCCFfunc(VolPerPMHIEharvest);
    const RelevanceIEharvest = intermediate.DBHST < 16 ? 1 : (intermediate.DBHST < 20 ? 5 - intermediate.DBHST / 4 : 0);
    // F) Norcar 600H (Brinker&Tufts, 90)
    const Stand1VolPerPMHIFharvest = Math.max(100, 85 * (-1.961 + 0.5502 * intermediate.DBHST
        + 1.871 * intermediate.TreeVolST - 0.04502 * intermediate.DBHST * intermediate.TreeVolST
        - 0.1786 * intermediate.CTLLogsPerTree * intermediate.TreeVolST)
        / (1 + intermediate.CSlopeFB_Harv + intermediate.CRemovalsFB_Harv));
    const Stand2VolPerPMHIFharvest = Math.max(100, 85 * (-1.539 + 0.6683 * intermediate.DBHST
        + 0.9754 * intermediate.TreeVolST - 0.1257 * intermediate.CTLLogsPerTree * intermediate.TreeVolST)
        / (1 + intermediate.CSlopeFB_Harv + intermediate.CRemovalsFB_Harv));
    ...
    const RelevanceIFharvest = 0.5 * (intermediate.DBHST < 15 ?
        1 : (intermediate.DBHST < 20 ? 4 - intermediate.DBHST / 5 : 0));
    ...
    const VolPerPMHIHharvest = (42.729 / 0.028317 * Math.pow(intermediate.TreeVolST * 0.028317, 0.8195)
    ...
    const RelevanceIHharvest = intermediate.TreeVolST < 30 ?
        1 : (intermediate.TreeVolST < 60 ? 2 - intermediate.TreeVolST / 30 : 0);
    ...
    const VolPerPMHIIharvest = (42.729 / 0.028317 * Math.pow(intermediate.TreeVolST * 0.028317, 0.8195)
    ...
    const DistPerMoveILharvest = 43560 / assumption.CTLTrailSpacing / intermediate.RemovalsST;
    const TimePerTreeILharvest = (0.188 - 0.155 * StandingDeadDummyILharvest - 0.217 * DownTreeDummyILharvest
        + 0.048 * DougFirDummyILharvest + 0.012 * DistPerMoveILharvest + 0.004 * Math.pow(intermediate.DBHST, 2))
        * (1 + intermediate.CSlopeFB_Harv);
    const VolPerPMHILharvest = VolPerPMHfunc(TimePerTreeILharvest);
    const CostPerCCFILharvest = CostPerCCFfunc(VolPerPMHILharvest);
    const RelevanceILharvest = intermediate.TreeVolST < 40 ?
        1 : (intermediate.TreeVolST < 80 ? 2 - intermediate.TreeVolST / 40 : 0);
    ...
    const DistPerMoveIMharvest = 43560 / assumption.CTLTrailSpacing / intermediate.RemovalsST;
    const TimePerTreeIMharvest = (0.35 - 0.107 * StandingDeadDummyIMharvest - 0.304 * DownTreeDummyIMharvest
        + 0.032 * DougFirDummyIMharvest + 0.01 * DistPerMoveIMharvest + 0.005 * Math.pow(intermediate.DBHST, 2))
        * (1 + intermediate.CSlopeFB_Harv);
    const VolPerPMHIMharvest = VolPerPMHfunc(TimePerTreeIMharvest);
    const CostPerCCFIMharvest = CostPerCCFfunc(VolPerPMHIMharvest);
    const RelevanceIMharvest = intermediate.TreeVolST < 40 ?
        1 : (intermediate.TreeVolST < 80 ? 2 - intermediate.TreeVolST / 40 : 0);
    ...
    const DistPerMoveINharvest = 43560 / assumption.CTLTrailSpacing / intermediate.RemovalsST;
    const TimePerTreeINharvest = (0.426 - 0.256 * StandingDeadDummyINharvest - 0.038 * DownTreeDummyINharvest
        + 0.085 * DougFirDummyINharvest + 0.011 * DistPerMoveINharvest + 0.005 * Math.pow(intermediate.DBHST, 2))
        * (1 + intermediate.CSlopeFB_Harv);
    const VolPerPMHINharvest = VolPerPMHfunc(TimePerTreeINharvest);
    const CostPerCCFINharvest = CostPerCCFfunc(VolPerPMHINharvest);
    const RelevanceINharvest = intermediate.TreeVolST < 40 ?
        1 : (intermediate.TreeVolST < 80 ? 2 - intermediate.TreeVolST / 40 : 0);
    ...
    const RelevanceIOharvest = intermediate.TreeVolST < 60 ?
        1 : (intermediate.TreeVolST < 100 ? 2.5 - intermediate.TreeVolST / 40 : 0);
    ...
    const TimePerTreeIPharvest = (0.232 + 0.0185 * 2.54 * intermediate.DBHST)
        * (1 + intermediate.CSlopeFB_Harv + intermediate.CRemovalsFB_Harv);
    ...
    const TimePerTreeIQharvest = (0.18 + 0.0038 * Math.pow(intermediate.DBHST, 2)
    ...
    const RelevanceIQharvest = intermediate.TreeVolST < 40 ?
        1 : (intermediate.TreeVolST < 80 ? 2 - intermediate.TreeVolST / 40 : 0);
    // R) User-Defined

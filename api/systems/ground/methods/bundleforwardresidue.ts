...
function BundleForwardResidue(assumption: AssumptionMod, input: InputVarMod,
                              intermediate: IntermediateVarMod, machineCost: MachineCostMod) {
    const BundleWeight = 0.9;
    const RecoveredResidue = assumption.ResidueRecovFracCTL * intermediate.ResidueST;
    ...
    const LoadIAbfr = (1 + 0.57) * (Math.max(3, 5.5 - 0.1 * (2.47 / 1.1) * RecoveredResidue)) / 1.1;
    ...
    const MoveDistIBbfr = (43560 * BundleWeight) / (assumption.CTLTrailSpacing * RecoveredResidue);
    ...
    const TotalIBbfr = ArrangeSlashIBbfr + FeedIBbfr + WaitIBbfr + TravelIBbfr + RotateBundlerIBbfr + CutBundleIBbfr;
    ...
    const TrabelIntermIIAbfr = (0.2048 + 0.0146 * 0.305 * DistPerMove) * MovesIIAbfr;
    const TravelLoadedIIAbfr = 0.239 + 0.0125 * 0.305 * DistIn;
    const UnloadIIAbfr = 5.385 / 2;
    const SortLandingIIAbfr = 0;
    const MoveLandingIIAbfr = (0.1763 + 0.0061 * 0.305 * LandingMoveDistPerMove) * LandingMovesIIAbfr;
    const TurnTimeIIAbfr = TravelOut + LoadIIAbfr + SortWoodIIAbfr + TrabelIntermIIAbfr + TravelLoadedIIAbfr
        + UnloadIIAbfr + SortLandingIIAbfr + MoveLandingIIAbfr;
    ...
    // Summary
    const CostBundleResidue = intermediate.ResidueST > 0 ?
        (machineCost.PMH_Bundler * RelevanceIAbfr + machineCost.PMH_Bundler * RelevanceIBbfr
            + machineCost.PMH_Bundler * RelevanceICbfr)
            / (RelevanceIAbfr * GT1Abfr + RelevanceIBbfr * GT1Bbfr + RelevanceICbfr * GT1Cbfr) : 0;
    const CostForwardResidueBundles = intermediate.ResidueST > 0 ?
        (machineCost.PMH_ForwarderS * RelevanceIIAbfr + machineCost.PMH_ForwarderS * RelevanceIIBbfr)
        / (RelevanceIIAbfr * GTIIAbfr + RelevanceIIBbfr * GTIIBbfr) : 0;

    return { 'CostBundleResidue': CostBundleResidue, 'CostForwardResidueBundles': CostForwardResidueBundles };
}

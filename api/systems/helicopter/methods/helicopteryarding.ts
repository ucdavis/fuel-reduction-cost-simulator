...
function HelicopterYarding(input: InputVarMod, intermediate: IntermediateVarMod) {
    ...
    const FlightTimeA = 6;
    const CCFperDay1A
        = calc1(ObservedAvgAtElevIA, Total1A, LoadUnload1A, YardingSpeed1A, FlightTimeA).CCFperDay;
    const CostPerCCF1A
        = calc1(ObservedAvgAtElevIA, Total1A, LoadUnload1A, YardingSpeed1A, FlightTimeA).CostPerCCF;
    const Relevance1A = intermediate.TreeVol < 75 ?
    ...
    const CCFperDay1B
        = calc1(ObservedAvgAtElevIB, Total1B, LoadUnload1B, YardingSpeed1B, FlightTimeB).CCFperDay;
    const CostPerCCF1B
        = calc1(ObservedAvgAtElevIB, Total1B, LoadUnload1B, YardingSpeed1B, FlightTimeB).CostPerCCF;
    const Relevance1B = intermediate.TreeVol < 75 ?
        1 : (intermediate.TreeVol < 150 ? 2 - intermediate.TreeVol / 75 : 0);
    // C) K-MAX
    ...
    const FlightTimeC = 8.50;
    const CCFperDay1C
        = calc1(ObservedAvgAtElevIC, Total1C, LoadUnload1C, YardingSpeed1C, FlightTimeC).CCFperDay;
    const CostPerCCF1C
        = calc1(ObservedAvgAtElevIC, Total1C, LoadUnload1C, YardingSpeed1C, FlightTimeC).CostPerCCF;
    const Relevance1C = intermediate.TreeVol < 75 ?
        1 : (intermediate.TreeVol < 150 ? 2 - intermediate.TreeVol / 75 : 0);
    // D) User-Defined
    ...
    // A) Bell 204 class
    const CCFperDay2A
        = calc2(ObservedAvgAtElevIA, Total1A, LoadUnload1A, YardingSpeed1A, FlightTimeA).CCFperDay;
    const CostPerCCF2A
        = calc2(ObservedAvgAtElevIA, Total1A, LoadUnload1A, YardingSpeed1A, FlightTimeA).CostPerCCF;
    const Relevance2A = intermediate.TreeVol < 75 ?
        1 : (intermediate.TreeVol < 150 ? 2 - intermediate.TreeVol / 75 : 0);
    // B) Boeing Vertol 107 - 61A
    const CCFperDay2B
        = calc2(ObservedAvgAtElevIB, Total1B, LoadUnload1B, YardingSpeed1B, FlightTimeB).CCFperDay;
    const CostPerCCF2B
        = calc2(ObservedAvgAtElevIB, Total1B, LoadUnload1B, YardingSpeed1B, FlightTimeB).CostPerCCF;
    const Relevance2B = intermediate.TreeVol < 75 ?
        1 : (intermediate.TreeVol < 150 ? 2 - intermediate.TreeVol / 75 : 0);
    // C) K-MAX
    const ObservedAvgAtElevIIC = -0.0046 * input.Elevation + 4800;
    const CCFperDay2C
        = calc2(ObservedAvgAtElevIIC, Total1C, LoadUnload1C, YardingSpeed1C, FlightTimeC).CCFperDay;
    const CostPerCCF2C
        = calc2(ObservedAvgAtElevIIC, Total1C, LoadUnload1C, YardingSpeed1C, FlightTimeC).CostPerCCF;
    const Relevance2C = intermediate.TreeVol < 75 ?
        1 : (intermediate.TreeVol < 150 ? 2 - intermediate.TreeVol / 75 : 0);
    // D) User-Defined
    ...
    // I. Manual Log-Length
    const CostHeliYardML = intermediate.CHardwood * (CCFperDay1A * CostPerCCF1A * Relevance1A
        + CCFperDay1B * CostPerCCF1B * Relevance1B + CCFperDay1C * CostPerCCF1C * Relevance1C
        + CCFperDay1D * CostPerCCF1D * Relevance1D) / (CCFperDay1A * Relevance1A + CCFperDay1B * Relevance1B
        + CCFperDay1C * Relevance1C + CCFperDay1D * Relevance1D);
    // II. CTL
    const CostHeliYardCTL = intermediate.CHardwood * (CCFperDay2A * CostPerCCF2A * Relevance2A
        + CCFperDay2B * CostPerCCF2B * Relevance2B + CCFperDay2C * CostPerCCF2C * Relevance2C
        + CCFperDay2D * CostPerCCF2D * Relevance2D) / (CCFperDay2A * Relevance2A + CCFperDay2B * Relevance2B
        + CCFperDay2C * Relevance2C + CCFperDay2D * Relevance2D);
    ...
    // I. Manual Log-Length
    const CostHeliLoadML = intermediate.CHardwood * (CCFperDay1A * CostPerCCF1A2 * Relevance1A
        + CCFperDay1B * CostPerCCF1B2 * Relevance1B + CCFperDay1C * CostPerCCF1C2 * Relevance1C
        + CCFperDay1D * CostPerCCF1D2 * Relevance1D) / (CCFperDay1A * Relevance1A + CCFperDay1B * Relevance1B
            + CCFperDay1C * Relevance1C + CCFperDay1D * Relevance1D);
    // II. CTL
    const CostHeliLoadCTL = intermediate.CHardwood * (CCFperDay2A * CostPerCCF2A2 * Relevance2A
        + CCFperDay2B * CostPerCCF2B2 * Relevance2B
        + CCFperDay2C * CostPerCCF2C2 * Relevance2C + CCFperDay2D * CostPerCCF2D2 * Relevance2D)
        / (CCFperDay2A * Relevance2A + CCFperDay2B * Relevance2B
            + CCFperDay2C * Relevance2C + CCFperDay2D * Relevance2D);

    const resultObj = { 'CostHeliYardML': CostHeliYardML, 'CostHeliYardCTL': CostHeliYardCTL,
                        'CostHeliLoadML': CostHeliLoadML, 'CostHeliLoadCTL': CostHeliLoadCTL };

    return resultObj;

    function calc1(ObservedAvgAtElev: number, Total: number, LoadUnload: number,
                   YardingSpeed: number, FlightTime: number) {
        const LogWtPerMaxPayload = LogWt / (1.25 * ObservedAvgAtElev);
        const MaxPayloadsPerHookArea = WtInHook_Area / (1.25 * ObservedAvgAtElev);
        const LoadAdjustFactor = (0.9 + (LogWtPerMaxPayload < 0.5 ?
            0.1 - 0.2 * LogWtPerMaxPayload : 0) + (MaxPayloadsPerHookArea < 4 ?
                -0.05 + (0.05 / 3) * (MaxPayloadsPerHookArea - 1) : 0)) * (MaxPayloadsPerHookArea < 1 ?
                    MaxPayloadsPerHookArea : 1);
    ...
    function calc2(ObservedAvgAtElev: number, Total: number, LoadUnload: number,
                   YardingSpeed: number, FlightTime: number) {
